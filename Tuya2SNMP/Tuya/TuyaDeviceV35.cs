...
namespace Tuya2SNMP.Tuya
{
    ...
    public class TuyaDeviceV35 : ITuyaDevice, IDisposable
    {
        ...
        private async Task HandleResponseAsync(TuyaLocalResponse response)
        {
            ...
            if (response.Command == TuyaCommand.HEART_BEAT)
            {
                return;
            }

            if (response.Command == TuyaCommand.STATUS)
            {
            ...
            if (response.Command == TuyaCommand.DP_QUERY_NEW)
            {
                if (string.IsNullOrEmpty(response.JSON))
                    return;
                var root = JObject.Parse(response.JSON);
            ...
            if (response.Command == TuyaCommand.SESS_KEY_NEG_RES)
            {
